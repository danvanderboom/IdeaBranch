...
public class TagTaxonomyViewProvider
{
    ...
    /// Returns empty collection if TreeView not yet initialized.
    /// </summary>
    public ObservableCollection<ITreeNode> ProjectedCollection
    {
        ...
        {
            if (_treeView == null)
                return new ObservableCollection<ITreeNode>();

            // Ensure projection is populated if the tree has children
            if (_treeView.ProjectedCollection.Count == 0 && _treeView.Root.Children.Count > 0)
            {
                _treeView.UpdateProjectedCollection();
            }
    ...
    public TagTaxonomyAdapter Adapter => _adapter;
        ...
        /// <summary>
        /// Forces the underlying TreeView to recompute the projected collection.
        /// </summary>
        public void Refresh()
        {
            if (_treeView == null)
